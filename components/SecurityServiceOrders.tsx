...
import { Plus, Edit, Eye, Share2, ClipboardCopy, UploadCloud, Image as ImageIcon, Video, X, Trash2, Printer, Undo2, Shield, Cctv, Lock, Bell, Search, Clock, Wrench, CheckCircle, MapPin, DollarSign } from 'lucide-react';
...
const statusColorMap: Record<ServiceOrderStatus, string> = {
  ...
  [ServiceOrderStatus.InProgress]: 'bg-purple-100 text-purple-700 border-purple-200',
...
const statusProgressMap: Record<ServiceOrderStatus, number> = {
    ...
    [ServiceOrderStatus.InProgress]: 50,
...
const SecurityServiceOrders: React.FC = () => {
  ...
  const filteredOrders = useMemo(() => {
      return serviceOrders.filter(order => {
          const matchesTab = 
            ...
            activeTab === 'pending' ? order.status === ServiceOrderStatus.WaitingParts :
  ...
  const stats = useMemo(() => {
      ...
      return { open, progress, completedCount: completed.length, revenue };
  }, [serviceOrders]);
  ...
  const getServiceIcon = (type: string) => {
      ...
      if (t.includes('cftv') || t.includes('camera')) return <Cctv size={20} />;
      if (t.includes('alarme')) return <Bell size={20} />;
      if (t.includes('acesso') || t.includes('interfone') || t.includes('fechadura')) return <Lock size={20} />;
      return <Shield size={20} />;
  };

  const columns = [
    { header: 'Serviço / Local', accessor: (item: ServiceOrder) => (
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <div className="flex items-center gap-1 text-xs text-gray-500 mt-0.5">
                    <MapPin size={10}/>
                    <span className="truncate max-w-[180px]" title={item.imeiOrSerial}>{item.imeiOrSerial || 'Endereço não inf.'}</span>
                </div>
            </div>
        </div>
    )},
         ...
         <div>
            ...
            {item.status}
             ...
             <div className="w-24 h-1.5 bg-gray-200 rounded-full mt-1.5 overflow-hidden">
                <div 
                    className={`h-full rounded-full ${item.status === ServiceOrderStatus.Canceled ? 'bg-red-400' : 'bg-purple-500'}`} 
  ...
  const statusSteps = [
      ...
      { id: ServiceOrderStatus.InProgress, label: 'Vistoria/Instalação' },
      { id: ServiceOrderStatus.WaitingParts, label: 'Pendência' },
      { id: ServiceOrderStatus.Completed, label: 'Finalizado' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                <Shield className="text-primary" /> O.S. - Segurança Eletrônica
            </h1>
            <p className="text-gray-500 text-sm">CFTV, Alarmes, Controle de Acesso e Instalações.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                  <p className="text-xs text-gray-500 font-semibold uppercase">Em Execução</p>
                  <p className="text-2xl font-bold text-purple-600">{stats.progress}</p>
              </div>
              <div className="p-2 bg-purple-50 rounded-lg text-purple-500"><Wrench size={24}/></div>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                  <p className="text-xs text-gray-500 font-semibold uppercase">Concluídos</p>
                  <p className="text-2xl font-bold text-emerald-600">{stats.completedCount}</p>
              </div>
              <div className="p-2 bg-emerald-50 rounded-lg text-emerald-500"><CheckCircle size={24}/></div>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
        ...
        <form onSubmit={handleFormSubmit} className="flex flex-col h-[80vh]">
            ...
            <div className="flex-1 overflow-y-auto px-1">
                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    {/* Left Column: Location & Client */}
                    <div className="lg:col-span-7 space-y-6">
                         {/* Location Card */}
                         <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                             <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center"><MapPin size={16} className="mr-2"/> Local da Instalação</h3>
                             <div className="grid grid-cols-2 gap-3 mb-3">
                                 <div className="col-span-1">
                                     <label className="text-[10px] uppercase font-bold text-gray-500">Tipo</label>
                                     <select name="deviceType" value={currentOrder?.deviceType || 'Sistema de CFTV'} onChange={handleInputChange} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg bg-white outline-none mt-1">
                                 ...
                                 <div className="col-span-1">
                                     <label className="text-[10px] uppercase font-bold text-gray-500">Equipamento (Marca)</label>
                                     <input type="text" name="deviceBrand" placeholder="Ex: Intelbras" value={currentOrder?.deviceBrand || ''} onChange={handleInputChange} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none mt-1" required/>
                                 </div>
                                 <div className="col-span-2">
                                      <label className="text-[10px] uppercase font-bold text-gray-500">Endereço Completo</label>
                                     <input type="text" name="imeiOrSerial" placeholder="Rua, Nº, Bairro" value={currentOrder?.imeiOrSerial || ''} onChange={handleInputChange} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none mt-1"/>
                                 </div>
                                 <div className="col-span-2">
                                      <label className="text-[10px] uppercase font-bold text-gray-500">Detalhes do Acesso / Obs. Local</label>
                                     <textarea name="accessories" placeholder="ex: Chave na portaria, acesso pelo telhado" value={currentOrder?.accessories || ''} onChange={handleInputChange} rows={2} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none mt-1"></textarea>
                                 </div>
                             </div>

                             {/* Visual Annotator */}
                             <div className="border rounded-lg bg-white p-2">
                                <p className="text-xs text-gray-500 mb-1 text-center">Planta / Pontos de Instalação</p>
                                <div className="h-40">
                                    {currentOrder && (
                                        <WireframeAnnotator 
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="md:col-span-2">
                                    <input type="text" name="customerName" placeholder="Nome Completo / Condomínio" value={currentOrder?.customerName || ''} onChange={handleInputChange} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none" required/>
                                </div>
                                <input type="tel" name="customerPhone" placeholder="Telefone / Responsável" value={currentOrder?.customerPhone || ''} onChange={handleInputChange} className="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none"/>
                         ...
                         {/* Files */}
                         <div>
                            <h3 className="text-sm font-bold text-gray-700 mb-2">Fotos do Local</h3>
                            <label htmlFor="file-upload-security" className="w-full flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm h-full flex flex-col">
                            <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center"><Wrench size={16} className="mr-2"/> Execução e Materiais</h3>
                            
                            <div className="flex-1 space-y-4">
                                <div>
                                    <label className="text-xs font-semibold text-gray-500">Solicitação / Escopo</label>
                                    <textarea name="reportedProblem" value={currentOrder?.reportedProblem || ''} onChange={handleInputChange} rows={3} className="w-full mt-1 p-2.5 text-sm border border-gray-300 rounded-lg outline-none resize-none" placeholder="O que deve ser feito?" required></textarea>
                                </div>
                                
                                <div>
                                    <label className="text-xs font-semibold text-gray-500">Relatório Técnico</label>
                                    <textarea name="technicianNotes" value={currentOrder?.technicianNotes || ''} onChange={handleInputChange} rows={3} className="w-full mt-1 p-2.5 text-sm border border-gray-300 rounded-lg outline-none resize-none bg-yellow-50" placeholder="Detalhes da execução..."></textarea>
                                </div>

                                <div>
                                    <label className="text-xs font-semibold text-gray-500">Equipamentos / Cabos Utilizados</label>
                                    <textarea name="partsUsed" value={currentOrder?.partsUsed || ''} onChange={handleInputChange} rows={4} className="w-full mt-1 p-2.5 text-sm border border-gray-300 rounded-lg outline-none resize-none" placeholder="Lista de materiais..."></textarea>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3 pt-2">
                                    ...
                                    <div>
                                        <label className="text-xs font-semibold text-gray-500">Materiais (R$)</label>
      ...
      {/* View Modal */}
       <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title={`O.S. Segurança #${selectedOrder?.id}`}>
        {selectedOrder && (
          <div className="text-sm text-gray-700 space-y-4 max-h-[80vh] overflow-y-auto pr-2">
             <div className="flex items-center justify-between bg-gray-50 p-4 rounded-lg">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                     ...
                     <p className="text-xs text-gray-500 mt-1"><MapPin size={10} className="inline mr-1"/>{selectedOrder.imeiOrSerial}</p>
                </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border p-3 rounded-lg">
                     <h4 className="text-xs font-bold text-gray-400 uppercase mb-1">Escopo / Solicitação</h4>
                     <p>{selectedOrder.reportedProblem}</p>
                </div>
                 <div className="border p-3 rounded-lg">
                     <h4 className="text-xs font-bold text-gray-400 uppercase mb-1">Relatório Técnico</h4>
            ...
            {selectedOrder.damageMarkers && selectedOrder.damageMarkers.length > 0 && (
                <div className="my-4 border rounded-lg p-2">
                    <h4 className="font-bold text-gray-800 mb-2 text-center">Planta Baixa / Pontos</h4>
                     <WireframeAnnotator 
