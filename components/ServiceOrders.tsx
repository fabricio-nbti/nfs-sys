    ...
    <div>
      ...
      <Modal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} title={selectedOrder ? "Editar Ordem de Serviço" : "Criar Nova Ordem de Serviço"}>
        <form onSubmit={handleFormSubmit} className="max-h-[80vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <select defaultValue={selectedOrder?.deviceType || 'Celular'} className="p-2 w-full border rounded-md bg-white">
                ...
                <input type="text" placeholder="IMEI ou Nº de Série" defaultValue={selectedOrder?.imeiOrSerial} className="p-2 w-full border rounded-md"/>
                <textarea placeholder="Acessórios (ex: carregador, cabo, capa)" defaultValue={selectedOrder?.accessories} rows={2} className="p-2 w-full border rounded-md col-span-2"></textarea>
            </div>
