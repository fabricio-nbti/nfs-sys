...
import { Plus, Edit, Trash2, CheckCircle, Search, Filter, AlertCircle, Clock, TrendingDown } from 'lucide-react';
...
const AccountsPayable: React.FC = () => {
  ...
  const { filteredTransactions, summary } = useMemo(() => {
    ...
    // Calculate Summaries based on ALL transactions (to give context)
    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
           ...
           <p className="text-gray-500 mt-1">Gerencie suas despesas e compromissos financeiros.</p>
        </div>
        <button 
          onClick={openModal}
          className="bg-red-600 text-white px-4 py-2 rounded-lg flex items-center hover:bg-red-700 transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-yellow-400 flex justify-between items-center">
            <div>
                <p className="text-sm text-gray-500 font-medium uppercase tracking-wider">A Pagar (Pendente)</p>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
         ...
         <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <select 
                ...
                className="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white flex-grow md:flex-grow-0"
            >
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        <DataTable<AccountTransaction>
            ...
            <div className="flex space-x-2">
                {item.status !== 'Paid' && <button className="p-1 text-green-600 hover:bg-green-50 rounded" title="Marcar como Pago"><CheckCircle size={18} /></button>}
        ...
        <form onSubmit={handleSave}>
          <div className="space-y-4">
            <div>
              ...
              <input type="text" name="description" id="description" value={editingTransaction.description || ''} onChange={handleInputChange} required className="mt-1 p-2 w-full border rounded-md focus:ring-red-500 focus:border-red-500"/>
            </div>
            <div>
              <label htmlFor="category" className="block text-sm font-medium text-gray-700">Categoria</label>
              <input type="text" name="category" id="category" value={editingTransaction.category || ''} onChange={handleInputChange} required className="mt-1 p-2 w-full border rounded-md focus:ring-red-500 focus:border-red-500"/>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="dueDate" className="block text-sm font-medium text-gray-700">Data de Vencimento</label>
                <input type="date" name="dueDate" id="dueDate" value={editingTransaction.dueDate || ''} onChange={handleInputChange} required className="mt-1 p-2 w-full border rounded-md focus:ring-red-500 focus:border-red-500"/>
              </div>
              <div>
                <label htmlFor="amount" className="block text-sm font-medium text-gray-700">Valor (R$)</label>
                <input type="text" name="amount" id="amount" value={formatCurrency(editingTransaction.amount)} onChange={handleCurrencyChange} required className="mt-1 p-2 w-full border rounded-md focus:ring-red-500 focus:border-red-500"/>
              </div>
            </div>
          </div>
          <div className="mt-6 flex justify-end">
            ...
            <button type="submit" className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Salvar</button>
          </div>
        </form>
      </Modal>
    </div>
